...
for (Int_t chindex = 0; chindex < 7; ++chindex){//Filling the Channel plots with each Amplitude
	...
	mgC[chindex]->SetMinimum(0);
        ...
        mgC[chindex]->SetTitle(Form("Channel %d Filtered",chindex));
...
canvasC->cd(9);
legendC->Draw();
canvasC->Print("Dec19_2/ChantitlesFiltered.png");
for (Int_t Ampindex = 0; Ampindex < 7; ++Ampindex){//Filling the Amplitude plots with each Channel
	...
	for(Int_t chindex = 0; chindex <7; ++chindex){
        ...
        mgA[Ampindex]->SetTitle(Form("Amplitude %d mV Filtered",NomAmp[Ampindex]));
...
legendA->Draw();
canvasA->Print("Dec19_2/AmptitlesFiltered.png");
